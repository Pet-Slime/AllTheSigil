...
namespace voidSigils
{
	public partial class Plugin
	{
		...
		private NewAbility AddPatchedBeesOnHit()
		{
			...
			const string rulebookDescription = "Once a card bearing this sigil is struck, a bee card is created for the owner if the struck card is in play. A bee is defined as: 1 power, 1 health, airborn.";
	...
	public class void_BeesOnHit : DrawCreatedCard
	{
		...
		public override IEnumerator OnTakeDamage(PlayableCard source)
		{
			...
			} else
            {
				...
				List<PlayableCard> queuedCards = new List<PlayableCard>(Singleton<Opponent>.Instance.Queue);
				opponentSlotsCopy.RemoveAll((CardSlot x) => queuedCards.Find((PlayableCard y) => y.QueuedSlot == x));
