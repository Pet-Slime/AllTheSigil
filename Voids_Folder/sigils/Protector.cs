			...
			if (opposingSlot.Card != null)
            {
				...
				if (adjacentSlots.Count > 0 && adjacentSlots[0].Index < opposingSlot.Index)
				{
					if (adjacentSlots[0].Card != null && !adjacentSlots[0].Card.Dead)
					{
						...
						Plugin.Log.LogMessage("protector test: ability: " + adjacentSlots[0].Card.Info.HasAbility(void_Protector.ability));
						if (adjacentSlots[0].Card.Info.HasAbility(void_Protector.ability))
                        {
							__result = true;
                        }
					}
